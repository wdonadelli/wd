	function wd_request_set(n) { /* define o número de requisições e decide sobre a janela modal */
		/* abrindo janela modal se estiver fechada e iniciar uma requisição */
		if (wd_request_counter === 0 && n > 0)
			document.body.appendChild(wd_modal_window);
		/* definir o contador de requisições */
		wd_request_counter =+ n > 0 ? 1 : -1;
		/* checar, após um tempo, sobre o fechamento da janela modal (para não ficar piscando) */
		window.setTimeout(function () {
			if (wd_request_counter < 1)
				return document.body.removeChild(wd_modal_window);
		}, 250);
...
/*----------------------------------------------------------------------------*/
	function wd_request(action, pack, callback, method, async) {
		...
		var data = {
			start:    0,        /* registra o início da requisição (tempo) */
			request:  null,     /* registra o objeto para requisições */
